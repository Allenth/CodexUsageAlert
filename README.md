...
- 按额度窗口计算可持续日均预算；若昨日有完整快照且未用完，剩余额度会结转到今日
- 仪表盘展示基础日均、昨日已用、结转和今日可用，并标明每项数据来源
...
日均预算按 `100 ÷ 额度窗口天数` 在本机计算。昨日已用来自应用保存的本机额度快照；只有“昨日可用预算”和“昨日已用”两项都完整时才会结转，公式为 `今日可用 = 基础日均 + max(0, 昨日可用 - 昨日已用)`（最高 100%）。缺少昨日记录时界面会显示“暂无快照”，不会把缺失数据当作零用量。
