...
可持续日均按 `100 ÷ 额度窗口天数` 在本机计算。昨日已用来自应用保存的本机额度快照，公式为 `今日上限 = 20% + max(0, 可持续日均 - 昨日已用)`（最高 100%）。例如七日窗口的日均约为 14.3%，昨日用了 7%，则昨日余量为 7.3%，今日上限为 27.3%。缺少昨日记录时界面会显示“暂无快照”，保持 20% 基础上限，不会把缺失数据当作零用量。

Token 每日数据以 `account/usage/read` 返回的 `dailyUsageBuckets` 为准。若最后一个日期早于本地今天，界面会显示实际截止日期并说明服务端日汇总尚未生成或同步，不会补一个 0 值作为今天用量。
